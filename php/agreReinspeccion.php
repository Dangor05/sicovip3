	...
	if(isset($_POST['nfin'])) {
	 if (isset($_POST['cedp'])) {
		...
		elseif ($opcion==2) {
			echo $apl;
					...
					if($query!=null){
						move_uploaded_file($sv04apl['tmp_name'],$path.$apl);
						mysqli_close($con);
						print "<script>alert(\"Exito!!.\");window.location='../inicio.php';</script>";
						
			}else{
				mysqli_close($con);
				print "<script>alert(\"A la Verga!!.\");window.location='../inicio.php';</script>";
		...
		mysqli_close($con);
				...
				mysqli_close($con);
				print "<script>alert(\"Mierda.\");window.location='../inicio.php';</script>";
		...
		elseif ($opcion==3) {
			...
			if($query1!=null){
				...
				mysqli_close($con);
				print "<script>alert(\"Verga.\");window.location='../inicio.php';</script>";
